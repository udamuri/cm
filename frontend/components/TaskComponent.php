...
class TaskComponent extends Component
{
	...
	//Yii::$app->mycomponent->getSetting($id);
	public function getSetting($id)
	{
		//$model = TableForum::findOne($id);
        ...
        if($query)
        {
        	return $query['setting_content'];
        }

        return null;
	}

	//Yii::$app->mycomponent->catalogImage($cid, $ext);
	public function catalogImage($cid, $ext)
	{

		$img = yii::getAlias('@frontend/web/product/'.$cid.$ext);
		$img_s = Yii::$app->homeUrl.'product/'.$cid.$ext.'?'.time();
		if(!file_exists($img))
		{
			$img_s = Yii::$app->homeUrl.'css/img/default.png?'.time();
		}
		
		return $img_s;
	}

	//Yii::$app->mycomponent->clientImage($cid, $ext);
	public function clientImage($cid, $ext)
	{

		$img = yii::getAlias('@frontend/web/client/'.$cid.$ext);
		$img_s = Yii::$app->homeUrl.'client/'.$cid.$ext.'?'.time();
		if(!file_exists($img))
		{
			$img_s = Yii::$app->homeUrl.'css/img/default.png?'.time();
		}
	...
	//Yii::$app->mycomponent->cutLabel($label);
	public function cutLabel($label = '', $length = 10)
	{
		$count = strlen($label);
		$print_label = $label;
		if($count > $length)
		{
			$print_label = substr (  $label , 0, $length ).'...';
		}
